...
import { LoadGameData, SaveGameData, LoadMeta, SaveMeta, RequireChapter, EndChapter } from './Module/GameData';
...
cc.Class({
    ...
    properties: {
        ...
        _prevF: null, //require到的prevFile
        _prevCh: null, //存档中的prevFile编号
        _prevID: null, //存档和运行中的prev Index
    ...
    onLoad() {
        ...
        this._typingAudio = cc.find("Canvas/AudioManager/Effects/TypeWriter_2").getComponent(cc.AudioSource);
        this._AudioManager = cc.find("Canvas/AudioManager");
        ...
        this.canvasNode.on(cc.Node.EventType.TOUCH_END, function() {
            if (this._revOn) {
                this.closeReview();
            } else {
        ...
        this.node.on('chapterLock', function() {
            ...
            cc.resources.load('Prefab/Popup Purchase Chap', function(err, res) {
                let root = cc.instantiate(res);
                this.canvasNode.addChild(root, 0, 'Purchase Chap');
            }.bind(this));

        }, this);

        this.node.on('buyChapterSuccess', function() {
            console.log('收到buyChapterSuccess事件');
            this.canvasNode.resumeSystemEvents(true);
            this._currChap = PurchaseChap.serial;
            this._currPlotIndex = 1;
            this._initCase = 1;

            this.handleRequire(this._currChap, 1, false, 'p');
            console.log('currchap ' + this._currChap);
            this.scheduleOnce(() => { this.node.emit('dialogEnd'); }, 0.25)
        ...
        this.handleRequire(f1, 1, true, 'p');
        this.handleRequire(f2, 2, true, 'a');

        //处理各个章节衔接关系
        var autoID = this._autoSaved.index;

        // 用于记录上次临时存下的PrevChapter的Index
        // 例如：上次在Attack，前一个章节是ChapterX，ID是XX
        // 只需要有prevID，不需要PrevChapter
        this._prevID = this._autoSaved.prevIndex;

        if (!this._isAttack) {
            this._currPlotIndex = autoID;
            this._currAttIndex = 1;
        } else {
            this._currAttIndex = autoID;
            this._currPlotIndex = this._prevID;
        };
    ...
    start() {
        ...
        if (this._isAttack) {
            ...
            console.log('content = plotF');
            content = this._plotF;
            id = this._currPlotIndex;
            if (!content) { console.log('content为空，错误！！！'); }
        };
    ...
    handleRequire(serial, type, loading, container) {
        ...
        let rs = RequireChapter(serial, type, loading);

        // container p plotF,  a attF

        if (rs[0] == 'SUCCESS') {
            console.log('require返回： ' + rs[0]);

            if (container == 'p') { this._plotF = rs[1] } else if (container == 'a') { this._attF = rs[1] };

        } else if (rs[0] == 'END') {
            if (container == 'p') { this._plotF = rs[1] } else if (container == 'a') { this._attF = rs[1] };
            if (loading) { this._currPlotIndex = this._plotF[0]['max'] };
            this._isAuto = 0;
            this.endGame(this, 4, this._afterEndGame)
        } else if (rs[0] == 'CHAPTERLOCK') {
            if (rs[2])
                this._currChap = rs[2];

            if (container == 'p') { this._plotF = rs[1] } else if (container == 'a') { this._attF = rs[1] };

            PurchaseChap.serial = this._currChap;
            this.node.emit('chapterLock');
        } else if (!rs[0]) {
            console.log('!!!require返回为空，错误');
        };
    ...
    labelWriter(text, nd, lb, speed) {
        ...
        //self._typingAudio.play();

        let func = setInterval(function() {
            if (!nd.active || self._isEnd) { return; }
    ...
    updateAutoSave(self, callback) {
        ...
        self._autoSaved.prevIndex = self._currPlotIndex;
    ...
    analyzeContent(callback) {
        ...
        if (self._isAttack) { // attack开
            ...
            if (id > content[0]['max']) {
                ...
                self._currChap++;
                let result = self.handleRequire(self._currChap, 1, false, 'p');

                if (result[0] == 'END' || result[0] == 'CHAPTERLOCK') {
                    callback = null;
                ...
                self.handleRequire(self._currAtt, 2, false, 'a');
                self._isAttack = true;
        ...
        if ((content[id] != null) && (content[id].Character != null)) {
            ...
            if (content[id].Music) {
                music = content[id].Music;
                console.log('=====读取到Music ：' + music);
            };
            if (content[id].FadeMusic) { fadeMusic = content[id].FadeMusic };
        ...
        class obj {
            constructor(sys, pos, ton, tp, tn, cn, emo, pch, ps, bg, st, sc, an, ev, so, eo, oa, ob, dec, as, bs, qu, cv, mt, at, md, fl, bch, mus, fm) {
                this.sys = sys;
                this.pos = pos;
                this.textOff = ton;
                this.type = tp;
        ...
        var info = new obj(sys, pos, textOff, type, textSwitch, charOff, emo, picChange, poSwitch, bg, st, switchap, anim, eVal, startOp, endOp, optA, optB, decOpt, aStr, bStr, quit, conv, meet, aTip, mode, float, blankChar, music, fadeMusic);
        self._info = info;

        if (stop) {
            self._isStop = true;
            self._textNode.active = false;
        } else { self._isStop = false };
    ...
    beforeDataUpdate(self, callback) {
        ...
        if (info.aTip) {
            ...
            cc.resources.load('Prefab/Attack_Notice', cc.Prefab, function(err, res) {
                let tips = cc.instantiate(res);
                self.canvasNode.addChild(tips, 0, 'Attack_Notice');
            }.bind(self));
        } else {
            let atipNode = self.canvasNode.getChildByName('Attack_Notice');
            if (atipNode) {
                atipNode.destroy();
            }
        };
        ...
        if (info.stamp) {
            stampPlayEnd = false;
            interval = 2.8;
            var picUrl = 'Pic/' + info.stamp;
            var stamp;

            cc.resources.load('Prefab/Stamp', cc.Prefab, (err, res) => {
                if (err) {
                    console.log('未能成功加载stamp Prefab');
                };
        ...
        if (info.switchap) {
            ...
            cc.resources.load('Prefab/Chap', cc.Prefab, (err, res) => {

                if (err) {
                    console.log('未能成功加载stamp Prefab');
                };
                var chap = cc.instantiate(res);
                chap.getComponentInChildren(cc.Label).string = info.switchap;
                self.canvasNode.addChild(chap, 0, 'Chap');

            });
        } else {
            let chap = self.canvasNode.getChildByName('Chap');
            if (chap) {
                console.log('======调整chap');
                chap.active = false;
                self.scheduleOnce(() => { chap.destroy() }, 1.5)
            }
        };
        ...
        if (info.textOff) { self._textNode.active = false } else {
            self._nameNode.active = !info.blankChar;
            let wg = self._dialogNode.getComponent(cc.Widget);
            if (info.blankChar) { wg.top = 45; } else { wg.top = 15 };
            wg.updateAlignment();
        };
    ...
    dataUpdate(self, info, callback) {
        ...
        if (info.bg || info.aTip) {
            id++;
            self.node.emit('dialogEnd');
            return;
        } else {
            if (content[id].Character.indexOf('EndOption') == -1) {
            ...
            //播放音乐
            if (info.music) {
                console.log('info.music存在');
                self._AudioManager.emit('playMusic', info.music)
            };
            if (info.fadeMusic) { self._AudioManager.emit('fadeMusic') };
    ...
    optChoose(choo, callback) {
        ...
        this._optNode.opacity = 0;
        this.scheduleOnce(() => {
            this._optNode.active = false;
            this._optNode.opacity = 255;
        }, 0.8);
    ...
    showSprite(self, picStr, info, callback) {
        ...
        var rescale = function(err, res, callback) {
            ...
            if (cc.isValid(res)) {
                ...
                var hList = [1.08, 0.8, 1, 0.9, 1, 1, 1, 0.87, 1, 0.9]; //头身一起的身高列表，单身体再乘系数
                ...
                if (part == 1) {
                    ...
                    if (pic == '901' || pic == '902') {
                        h = self.canvasNode.height * 0.9 * 0.68;
                        w = h * ratio;
                    } else if (pic == '904') {
    ...
    _setClick(self) {
        var interval;
        if (self._isAuto == 1) { interval = 0.8 } else if (self._isAuto == 2) { interval = 0.6 } else { interval = 0; };
        ...
        // 每0.5秒检查一次，是否typing。没有则触发
        var lastTypingState = true; //默认在typing
        let click = setInterval(function() {
            if (self._isAuto == 0) { clearInterval(click) } else {
                if (lastTypingState && !self._isTyping) {
                    //上一次是true，这一次是not typing（typing状态切换）
                    console.log('dialog长度：' + self._dialogStr.length);
                    lastTypingState = false;

                    let wait = 0;
                    if (self._dialogStr.length <= 20) { wait = interval } else { wait = 1.5 * interval };
