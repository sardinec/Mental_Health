...
class rolling_lottery extends cc.Component {
    ...
    private _comp_move_dist(index_n_: number): number {
        ...
        console.log('comp_move_dist:' + dist_y_n);
     ...
     * @param index_n_ 目标位置
     */
    public scroll(index_n_: number,audio:cc.AudioSource, config_o_?: rolling_lottery.config):any {
        ...
        console.log('move_dist_n'+move_dist_n);
        let soundCount = 0;
        // 惯性距离
     ...
     */
    public jump(index_n_ = Math.floor(Math.random() * (this.node.children.length + 1))): any {
        /**运动距离 */
        let move_dist_n = this._comp_move_dist(index_n_);
        console.log('move_dist_n '+ move_dist_n);
        this._curr_speed_n = this._scroll_action_a.dist_n = move_dist_n;
        this._curr_speed_n  = move_dist_n;
        
        
        let result = this._scrolling();
        return result;
    }
    /* ***************其他事件*************** */
