...
export default class BhvBullet extends cc.Component {
    ...
    onLoad () {
        ...
        if(!this.isLockAngle){
            this.dx = Math.cos((-this.node.angle) /180 * Math.PI) * speed;
            this.dy = Math.sin((-this.node.angle) /180 * Math.PI) * speed;
        }else{
		...
		this.lastKnownAngle = -this.node.angle;
		this.travelled = 0;
    ...
    update (dt:number) {
		...
		if (this.node.angle !== -this.lastKnownAngle)
		{
			if (!this.isLockAngle)
			{   
				s = distanceTo(0, 0, this.dx, this.dy);
				this.dx = Math.cos((-this.node.angle) /180* Math.PI) * s;
				this.dy = Math.sin((-this.node.angle) /180* Math.PI) * s;
			}
		...
		if (this.dx !== 0 || this.dy !== 0)
		{
			...
			if (!this.isLockAngle)
			{
				this.node.angle = -angleTo(0, 0, this.dx, this.dy);
				this.lastKnownAngle = -this.node.angle;
			}
